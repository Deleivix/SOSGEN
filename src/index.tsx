...
import { initSessionManager } from './utils/session';
...
const pageRenderers = [
    ...
    (container: HTMLElement) => renderAdminPage(container),
    (container: HTMLElement) => renderSupervisorPage(container),
    (container: HTMLElement) => renderProfile(container),
];
...
async function checkBadges(user: User) {
    ...
    try {
        ...
        if (drillBadge) drillBadge.style.display = pending > 0 ? 'block' : 'none';
    } catch(e) {}
}
...
function renderMainApp(user: User) {
            ...
            <div class="nav-bottom">
                ...
                <div class="nav-links-container">
                    ${APP_PAGES.map((page, index) => {
                        if (page.name === 'HOME' || page.name === 'PERFIL') return '';
                        if (page.name === 'ADMIN' && !user.isAdmin) return '';
                        if (page.name === 'SUPERVISOR' && !(user.isAdmin || user.isSupervisor)) return '';
                        return `
                ...
                <div class="nav-right-controls">
                    ...
                    <div class="user-session-controls">
                        <span class="nav-user-display" id="user-profile-link" style="cursor: pointer; position: relative;" title="Ver Perfil">
                            ${user.username}
                            <span id="user-msg-badge" style="display:none; position: absolute; top: -5px; right: -10px; width: 8px; height: 8px; background-color: var(--accent-color); border-radius: 50%;"></span>
                        </span>
    ...
    showTipOfTheDay();
    checkBadges(user);
    setInterval(() => checkBadges(user), 60000); // Check badges every minute
}
...
function addMainAppEventListeners() {
    ...
    container.addEventListener('click', (event) => {
        ...
        if (profileLink) switchToPage(12); // Profile Page Index
    });
